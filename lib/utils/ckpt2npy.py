...
def save_tensors_in_npy(file_name, tensor_name, all_tensors, np_path):
    try:
        reader = pywrap_tensorflow.NewCheckpointReader(file_name)
        var_to_shape_map = reader.get_variable_to_shape_map()
        data = {}
        for key in var_to_shape_map:
            ...
            print("tensor_name: ", tensorname, ", sub_name: ", sub_name)
            if tensorname in data:
                data[tensorname][sub_name] = array
            else:
                data[tensorname] = {}
                data[tensorname][sub_name] = array
        with open(np_path, 'wb') as data_out:
...
def main(unused_argv):
    if not FLAGS.file_name:
        ...
        save_tensors_in_npy(FLAGS.file_name, FLAGS.tensor_name, FLAGS.all_tensors, FLAGS.npy_path)
